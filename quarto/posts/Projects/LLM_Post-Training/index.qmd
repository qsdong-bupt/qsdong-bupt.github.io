...
| `reward` | `float` | 综合奖励 = 格式正确且答案正确为 1，其余为 0 |
...
- `reward` 为二值综合奖励：格式正确且答案正确时返回 1，其余情况（格式错误、或格式正确但答案错误）均返回 0。注意它不是 `format_reward + answer_reward` 的求和——代码中格式正确但答案错误时返回 `reward: 0.0`，而非 1.0。即只有同时满足格式和答案两个条件，`reward` 才为 1。
...
| 指标 | 零样本基线 | SFT-raw | GRPO |
|------|:------:|:------:|:------:|
| 答案正确率 | 2.6% | 18.6% | 34.6% |
| 格式正确率 | 17.3% | 61.3% | 88.8% |
...
**关于 `train/grad_norm`。** 代码使用 `clip_grad_norm_(max_norm=1.0)`，图中所有值均为裁剪前的原始范数，公式为 $\text{grad\_norm} = \sqrt{\sum_{p} \|\nabla_p \mathcal{L}\|_2^2}$。

**关键观察：**

1. **loss 量级极低且非单调。** 稳定时 loss 约为 $10^{-5} \sim 10^{-3}$，可正可负。GRPO 的 loss = $-\min(r_t A_t, \text{clip}(r_t)A_t)$ 并非 SFT 的 NLL（恒正），其绝对值与梯度范数无单调关系——loss 小不意味着梯度小，反之亦然。

2. **梯度爆炸的根因：`min` 操作的不对称性。** 130 步中共 12 次 grad_norm > 1.0 的尖峰，最严重为 step 121（14.8M，伴随 loss=332、loss_std=664）。`clip(r_t)` 本身将 importance sampling 比率限制在 $[0.8, 1.2]$ 内，工作正常。问题在于 `min` 的二选一逻辑在两个方向上不对称：

   - **$A_t > 0$（好 token，鼓励）**：$r_t$ 膨胀时，$\min(r_t A_t,\; 1.2 A_t)$ 选 $1.2 A_t$，更新幅度被限制在 $1.2|A_t|$ 以内。裁剪生效。
   - **$A_t < 0$（坏 token，惩罚）**：$r_t$ 膨胀时，$\min(r_t A_t,\; 0.8 A_t)$ 两个都是负数，`min` 必然选**更负**的那个——即 $r_t A_t$。CLIP 把 surr2 压到了 $0.8 A_t$（例如 −3.12），但 $r_t A_t$ 可能是 −2,600,000，`min` 直接绕过裁剪值。

   爆炸的完整链条：离策略训练（每批 2 epoch）→ 策略漂移 → $r_t = e^{15} \approx 3.3\times 10^6$ → 负优势下 `min` 绕过 clip → per-token loss = $|r_t A_t| \approx 2.6\times 10^6$（正常值 ~0.1）→ 28 层链式法则逐层放大 → 梯度范数膨胀至 $10^7$ 量级。

3. **梯度裁剪的局限。** `clip_grad_norm_` 将范数压到 1.0，但无法修正方向——若梯度方向已被单一异常 token 主导，裁剪后的更新仍然指向噪声。模型每次爆炸后均能自动恢复，说明异常 token 方向的 loss landscape 足够平坦，且范数压到 1.0 后实际更新幅度极小。

4. **后期尖峰频率增加。** 前 60 步无 >1.0 尖峰；60–90 步 3 次；90–130 步 10 次。策略离初始点越远，ratio 方差越大，极端值概率越高。
...
| 指标 | 零样本基线 | SFT-raw | GRPO |
|:---|:---:|:---:|:---:|
| `avg_reward` | 0.0258 | 0.1860 | **0.3458** |
| `avg_format_reward` | 0.1728 | 0.6134 | **0.8880** |
| 条件正确率 | 14.9% | 30.3% | **~38.9%** |
